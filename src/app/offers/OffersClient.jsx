          ...
          <div className={styles.grid}>
            <div className={styles.list}>
              ...
              <div className={styles.dateRow}>
                ...
                <span className={styles.dateLabel}>{obj?.depart?.date ? parseLocalDate(obj.depart.date)?.toLocaleDateString('fr-FR', { weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' }) : ''}</span>
              </div>
                          ...
                          <React.Fragment key={idx}>
                            <div className={styles.segment}>
                              ...
                              <div className={styles.segmentRight}>
                                ...
                                {/* Display rolling stock image only if segment has a materiel with image_path AND the segment is on the search date */}
                                {(() => {
                                  try {
                                    const searchDateStr = obj?.depart?.date || null;
                                    if (!searchDateStr) return null;

                                    // Parse the search date as local date
                                    const searchDate = parseLocalDate(searchDateStr);
                                    if (!searchDate) return null;

                                    const searchDateOnly = new Date(searchDate.getFullYear(), searchDate.getMonth(), searchDate.getDate());

                                    const segTime = s.depart || '';
                                    if (!segTime) return null;

                                    // Create segment date with the search date and segment time
                                    const [sh, sm] = String(segTime).split(':').map(Number);
                                    const segDate = new Date(searchDate.getFullYear(), searchDate.getMonth(), searchDate.getDate(), sh || 0, sm || 0, 0);

                                    // Check if segment is on the same day as search (within 24h from midnight)
                                    const timeDiff = segDate.getTime() - searchDateOnly.getTime();
                                    const within24h = timeDiff >= 0 && timeDiff < 24 * 3600 * 1000;

                                    if (within24h && s.materiel && s.materiel.image_path) {
                                      // ensure we build an absolute or site-relative path
                                      const img = s.materiel.image_path.startsWith('/') ? s.materiel.image_path : `/${s.materiel.image_path}`;
                                      return (
                                        <div className={styles.trainImageContainer}>
                                          <img src={img} alt={s.materiel.nom || 'Matériel roulant'} className={styles.trainImage} />
                                        </div>
                                      );
                                    }
                                  } catch (e) { /* ignore */ }
            ...
            <aside className={styles.summary}>
              <div className={styles.summaryCard}>
                <div>Aller le {obj?.depart?.date ? parseLocalDate(obj.depart.date)?.toLocaleDateString('fr-FR', { weekday: 'short', day: '2-digit', month: 'short', year: 'numeric' }) : ''} à {obj?.depart?.time || ''}</div>
